...
Cuando haces dnormales[0:3] en R, estás tratando de extraer un subconjunto de elementos del vector dnormales que van desde la posición 0 hasta la posición 3. Sin embargo, en R, las posiciones de los elementos de un vector comienzan en 1 y no en 0. Por lo tanto, esto generará un vector vacío, ya que no hay ningún elemento en la posición 0.
...
Para obtener el único valor lógico de listaPrueba, que está dentro de la lista "c", puedes hacerlo así:
...
Si haces listaPrueba[[3]]$c[[2]], no es equivalente a listaPrueba$casos[[3]][[2]]. La razón es que listaPrueba[[3]] devuelve la lista "c" y luego $c[[2]] intenta acceder al segundo elemento de "c" que no existe. Por lo tanto, esto generaría un error. Por otro lado, listaPrueba$casos[[3]][[2]] accede al tercer elemento de "casos" (que es una lista) y luego al segundo elemento de esa lista.
